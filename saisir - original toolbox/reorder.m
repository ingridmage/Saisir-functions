function [saisir1, saisir2]=reorder(s1,s2,output)
%reorder 			- reorders the data of files s1 and s2 according to their identifiers
%function [B1 B2]=reorder(A1,A2,output)
...
for row=1:nrow
   flag=0;
   for row1=1:nrow1
      if(strcmp(strtrim(model(row1,:)),strtrim(s1.i(row,:)))==1)
	...
	if(flag==0)
		if output==1; disp(['Not found from s1: ' s1.i(row,:)]); end
	end
 end;
 nrealised=sum(detected>0);
 if output==1; disp(['number of matching samples: ' num2str(nrealised)]); end
 ...
 if(sum(detected==0)>0)
	if output==1; disp('Not found from s2'); end
 	bid(detected==0,:)
 end;   
 %newmodel;
 ...
 for row1=1:nmatch
    flag=0;
    for row=1:nrow
    	if(strcmp(strtrim(newmodel(row1,:)),strtrim(s1.i(row,:)))==1)
      	indice=indice+1;
